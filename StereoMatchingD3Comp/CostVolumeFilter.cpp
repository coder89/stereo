...
CostVolumeFilter::CostVolumeFilter(ID3D11Device1 * device)
{
	m_loadingComplete = false;
	m_d3dDevice = device;
}

void CostVolumeFilter::Initialize(uint32 width, uint32 height)
{
	m_width = width;
	m_height = height;
	...
	auto loadVSTask = DX::ReadDataAsync("CostFilteringVertexShader.cso");
	auto loadPSTask = DX::ReadDataAsync("CostFilteringPixelShader.cso");
...
void CostVolumeFilter::SetLeftTexture(ID3D11Texture2D * texture, ID3D11ShaderResourceView * textureView)
{
	m_textureLeft = texture;
	m_textureLeftView = textureView;
}

void CostVolumeFilter::SetRightTexture(ID3D11Texture2D * texture, ID3D11ShaderResourceView * textureView)
{
	m_textureRight = texture;
	m_textureRightView = textureView;
}

ID3D11ShaderResourceView * * CostVolumeFilter::GetShaderResourceView()
{
	return m_renderShaderResourceView.GetAddressOf();
}

void CostVolumeFilter::Render(ID3D11DeviceContext * context)
{
