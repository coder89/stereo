...
float4 main(PixelShaderInput input) : SV_TARGET
{
	...
	float4 px0_right = TextureRight.Sample(Sampler, dispTex - float2(dx, 0.0f));
	float4 px1_right = TextureRight.Sample(Sampler, dispTex);
	float4 px2_right = TextureRight.Sample(Sampler, dispTex + float2(dx, 0.0f));
